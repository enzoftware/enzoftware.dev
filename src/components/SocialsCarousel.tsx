...
const socials = [
  ...
  {
    label: "Substack",
    url: "https://enzoftware.substack.com/",
    icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
...
interface SocialsCarouselProps {
  t: Translations["social"];
}

export function SocialsCarousel({ t }: SocialsCarouselProps) {
  return (
    <motion.div
    ...
    >
      <h2 className="text-sm font-semibold text-ink mb-3">{t.title}</h2>
      <div className="flex gap-2 overflow-x-auto scrollbar-none -mx-1 px-1">
        {socials.map((social, i) => (
          <motion.a
            ...
            className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl border border-border text-ink-muted hover:text-accent hover:border-accent/30 transition-colors flex-shrink-0"
